...
namespace MediAssist_Desktop_App.Model
{
    public class AppointmentsModel
    {
		...
		public List<Appointment> getTableForConsumer(int id, int status)
		{
			List<Appointment> table = new List<Appointment>();
			...
			if (status == 0)
			{
				query = "SELECT * FROM appointments WHERE consumer_id = '" + id + "' ORDER BY appointment_status;";
			}

			else
			{
				query = "SELECT * FROM appointments WHERE consumer_id = '" + id + "' AND appointment_status = '" + status + "';";
		...
		{
			string query = "INSERT INTO appointments VALUES ('" + appointment.Date + "', '" + appointment.Issue + "', '" + appointment.Doctor_id + "', '" + appointment.Consumer_id + "', '" + appointment.Appointment_status + "');";
